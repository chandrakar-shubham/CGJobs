...
<div class="max-w-7xl mx-auto space-y-6">
  <div><h2 class="text-2xl font-bold text-slate-900">Jobs API & Website Sources</h2><p class="text-sm text-slate-500 mt-1">Websites se jobs automatically fetch, review aur publish karein.</p></div>
  ...
  <div class="bg-white border rounded-2xl p-6 shadow-sm">
    ...
    <form method="POST" action="{{route('admin.job-sources.store')}}" class="grid md:grid-cols-2 gap-4">@csrf
      ...
      <select name="frequency_minutes" class="border rounded-xl p-3"><option value="15">Every 15 minutes</option><option value="30">Every 30 minutes</option><option value="60" selected>Every hour</option><option value="120">Every 2 hours</option><option value="360">Every 6 hours</option><option value="720">Every 12 hours</option><option value="1440">Daily</option></select>
      ...
      <input name="default_category" placeholder="Default category (optional)" class="border rounded-xl p-3">
