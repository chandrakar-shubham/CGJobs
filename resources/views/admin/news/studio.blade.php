...
<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
  <div><h2 class="text-2xl font-bold">News / Current Affairs Studio</h2><p class="text-sm text-slate-500 mt-1">Fetch → keep only the news you want → build a persistent AI batch → process → review → publish.</p></div>
...
<div class="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-5">
 @foreach([['fetched','Fetched / Review','amber'],['batch','AI Batch',count($batchIds)],['processed','AI Processed','blue'],['published','Published','emerald'],['archived','Archived','slate']] as $s)<div class="bg-white rounded-2xl border p-4"><div class="text-xs text-slate-500">{{$s[0]==='batch'?$s[1]:$s[1]}}</div><div class="text-2xl font-bold {{is_string($s[2])?'text-'.$s[2].'-600':'text-purple-600'}}">{{$s[0]==='batch'?count($batchIds):$stats[$s[0]]}}</div></div>@endforeach
</div>
<div class="flex flex-wrap gap-2 mb-4">
 @foreach([['fetched','1. Fetched News'],['processed','2. AI Processed'],['all','3. All News']] as $tab)<a href="{{route('admin.news.index',['stage'=>$tab[0]])}}" class="px-4 py-2 rounded-xl text-sm font-semibold {{ $stage===$tab[0]?'bg-slate-900 text-white':'bg-white border text-slate-700' }}">{{$tab[1]}}</a>@endforeach
</div>

@if($stage==='fetched')
<div class="bg-white rounded-2xl border p-5 mb-5">
 <div class="flex items-center justify-between mb-3"><div><h3 class="font-bold">1. Fetch News for Review</h3><p class="text-xs text-slate-500">Fetching never calls AI. You can fetch repeatedly; duplicates are ignored and your existing AI Batch is preserved.</p></div><span class="text-xs px-2 py-1 rounded-lg bg-slate-100">Google Trending / Google News / NewsData / NewsAPI</span></div>
 <form method="POST" action="{{route('admin.news.fetch')}}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-3">@csrf
  ...
  <div><label class="text-xs font-semibold">Articles per fetch</label><select name="limit" class="mt-1 w-full rounded-xl border px-3 py-2 text-sm"><option>10</option><option selected>20</option><option>30</option><option>50</option><option>100</option></select></div>
...
<div class="bg-white rounded-2xl border p-4 mb-4 flex flex-wrap items-center justify-between gap-3">
 <div><h3 class="font-bold">{{ $stage==='fetched'?'Review Queue':($stage==='processed'?'2. AI Processed News':'3. All News') }}</h3><p class="text-xs text-slate-500">{{ $stage==='fetched'?'Delete unwanted articles. Keep selecting across multiple fetches until your target (for example 51) is reached.':'The same News record contains both website and mobile/app versions.' }}</p></div>
 @if(count($batchIds))<div class="flex items-center gap-2"><span class="px-3 py-2 rounded-lg bg-purple-50 text-purple-700 text-xs font-semibold">AI Batch: {{count($batchIds)}} selected</span><form method="POST" action="{{route('admin.news.batch-clear')}}">@csrf<button class="px-3 py-2 rounded-lg bg-slate-100 text-slate-700 text-xs">Clear Batch</button></form><form method="POST" action="{{route('admin.news.batch-process')}}">@csrf<button class="px-4 py-2 rounded-lg bg-purple-700 text-white text-xs font-semibold">Process Entire AI Batch</button></form></div>@endif
</div>

@if($stage==='fetched')
<form method="POST" action="{{route('admin.news.batch-add')}}">@csrf
 ...
 <div class="bg-white rounded-2xl border overflow-hidden">
  <div class="p-3 border-b bg-slate-50 flex flex-wrap items-center justify-between gap-2">
   ...
   <div class="flex gap-2">
    @if($stage==='fetched')<button type="submit" class="px-4 py-2 rounded-lg bg-amber-500 text-white text-xs font-semibold">Add Selected to AI Batch</button>
    @elseif($stage==='processed')<button type="submit" class="px-4 py-2 rounded-lg bg-emerald-600 text-white text-xs font-semibold">Publish Selected → Website + App</button>@endif
   </div>
  </div>
  ...
  @forelse($news as $item)
   <tr class="hover:bg-slate-50 align-top"><td class="p-3"><input name="ids[]" value="{{$item->id}}" type="checkbox" class="news-check h-4 w-4"></td><td class="p-3">@if($item->image_url)<img src="{{$item->image_url}}" alt="" class="w-20 h-14 object-cover rounded-lg">@else<div class="w-20 h-14 rounded-lg bg-slate-100 flex items-center justify-center text-[10px] text-slate-400">No image</div>@endif</td><td class="p-3 min-w-[360px]"><div class="font-semibold">{{Str::limit($item->title,110)}}</div><div class="text-xs text-slate-500 mt-1">{{Str::limit(strip_tags($item->content ?: $item->summary),260)}}</div><div class="text-[11px] text-slate-400 mt-2">ID {{$item->id}} · {{$item->published_at?->format('d M Y H:i')}}</div></td><td class="p-3 min-w-[150px]">{{$item->source ?: 'Unknown'}} @if($item->original_url)<a target="_blank" rel="noopener" href="{{$item->original_url}}" class="block text-blue-600 text-xs mt-1">Original ↗</a>@endif</td><td class="p-3"><span class="px-2 py-1 rounded-lg bg-slate-100 text-xs">{{$item->category}}</span></td><td class="p-3"><span class="px-2 py-1 rounded-lg text-xs font-semibold {{ $item->status==='published'?'bg-emerald-50 text-emerald-700':($item->status==='archived'?'bg-slate-100 text-slate-600':'bg-amber-50 text-amber-700') }}">{{ucfirst($item->status)}}</span>@if($item->status==='published')<div class="text-[11px] mt-2 space-y-1"><div>Web: {{$item->published_web?'✓':'—'}}</div><div>App: {{$item->published_mobile?'✓':'—'}}</div></div>@endif</td><td class="p-3"><div class="flex flex-wrap justify-end gap-1.5"><a href="{{route('admin.news.view',$item)}}" class="px-2.5 py-1.5 rounded-lg bg-slate-100 text-slate-700 text-xs">View</a><a href="{{route('admin.news.edit',$item)}}" class="px-2.5 py-1.5 rounded-lg bg-blue-50 text-blue-700 text-xs">Edit</a>@if($stage==='all' && $item->status==='published')<form method="POST" action="{{route('admin.news.archive',$item)}}">@csrf<button class="px-2.5 py-1.5 rounded-lg bg-slate-100 text-slate-700 text-xs">Archive</button></form><form method="POST" action="{{route('admin.news.channel',$item)}}">@csrf<input type="hidden" name="channel" value="web"><input type="hidden" name="action" value="{{$item->published_web?'unpublish':'publish'}}"><button class="px-2.5 py-1.5 rounded-lg {{ $item->published_web?'bg-amber-50 text-amber-700':'bg-blue-50 text-blue-700' }} text-xs">{{$item->published_web?'Unpublish Web':'Publish Web'}}</button></form><form method="POST" action="{{route('admin.news.channel',$item)}}">@csrf<input type="hidden" name="channel" value="mobile"><input type="hidden" name="action" value="{{$item->published_mobile?'unpublish':'publish'}}"><button class="px-2.5 py-1.5 rounded-lg {{ $item->published_mobile?'bg-amber-50 text-amber-700':'bg-purple-50 text-purple-700' }} text-xs">{{$item->published_mobile?'Unpublish App':'Publish App'}}</button></form>@endif<form method="POST" action="{{route('admin.news.studio-delete',$item)}}" onsubmit="return confirm('Delete this article permanently?')">@csrf @method('DELETE')<button class="px-2.5 py-1.5 rounded-lg bg-rose-50 text-rose-700 text-xs">Delete</button></form></div></td></tr>
...
<script>
const refresh=()=>{const n=document.querySelectorAll('.news-check:checked').length;document.getElementById('selectedCount').textContent=n+' selected';};
