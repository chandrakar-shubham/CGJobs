...
class JobImportNormalizer
{
    public function normalize(JobImport $import): JobImport
    {
        ...
        try {
            ...
            // Published by is an internal CGJobs classification, never the scraping website.
            ...
            if ($import->status === 'published' && $import->job) {
                ...
                $job->update([
                    ...
                    'published_by' => $updates['published_by'] ?? ($job->published_by ?: $job->job_category),
                ]);
            }
        } catch (\Throwable $e) {
            // A normalization failure must not prevent the rest of the import batch.
            report($e);
        }
    ...
    private function extractTitleAndPublisher(string $html): array
    {
        ...
        foreach ($xpath->query('//article//h1 | //main//h1 | //h1') as $node) {
            $titleCandidates[] = $this->clean($node->textContent);
        }
        ...
        foreach ($xpath->query('//meta[@property="og:title"] | //meta[@name="twitter:title"] | //title') as $node) {
            $titleCandidates[] = $this->clean($node->getAttribute('content') ?: $node->textContent);
        }
    ...
    private function collectJsonLdHeadlines($value, array &$out): void
    {
        ...
        if (!empty($value['headline']) && is_string($value['headline'])) $out[] = $this->clean($value['headline']);
