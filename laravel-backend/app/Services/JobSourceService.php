...
class JobSourceService
{
    public function sync(JobSource $source): array
    {
        ...
        try {
            $html=$this->get($source->fetch_url);
            $items=$this->parseListing($html,$source);
            $imported=$skipped=0;
            foreach($items as $item){
                ...
                $detail=$this->parseDetail($this->get($item['url']),$item['url']);
                $item=array_merge($item,$detail);
                $import=JobImport::create([
                    'job_source_id'=>$source->id,'external_key'=>sha1($item['url']),'external_url'=>$item['url'],
                    'title'=>Str::limit(trim($item['title']),250,''),'summary'=>$item['summary']??null,'content'=>$item['content']??$item['summary']??null,
                    'category'=>$item['category']??$source->default_category,'image_url'=>$item['image_url']??null,'published_at'=>$item['published_at']??null,
    ...
    public function publish(JobImport $import): Job
    {
        ...
        $job=Job::create(['title'=>$title,'title_en'=>$title,'summary'=>$summary,'summary_en'=>$summary,'detailed_content'=>$content,'detailed_content_en'=>$content,
            'category'=>$import->category?:'CG Vyapam','category_en'=>$import->category?:'CG Vyapam','section'=>'jobs','post_type'=>'job','source'=>$import->source->name,
            'source_url'=>$import->external_url,'image_url'=>$import->image_url,'published_at'=>optional($import->published_at)->format('Y-m-d')?:now()->format('Y-m-d'),
            'relative_time'=>'हाल ही में','relative_time_en'=>'Recently','is_new'=>true,'application_start'=>'जारी','last_date'=>'शीघ्र','translation_status'=>'pending']);
        $import->update(['job_id'=>$job->id,'status'=>'published','processed_at'=>now()]);
        return $job;
    }

    private function get(string $url): string
    { return Http::timeout(30)->retry(2,1000)->withHeaders(['User-Agent'=>'Mozilla/5.0 (compatible; CGJobsBot/1.0)'])->get($url)->throw()->body(); }
    ...
    private function parseListing(string $html, JobSource $source): array
    {
        ...
        foreach($xpath->query('//a[@href]') as $a){$title=trim(preg_replace('/\s+/u',' ',$a->textContent));$href=trim($a->getAttribute('href'));if(!$href||str_starts_with($href,'#'))continue;
            if(str_starts_with($href,'/'))$href=rtrim($source->base_url,'/').$href;
            if(mb_strlen($title)<12||!filter_var($href,FILTER_VALIDATE_URL)||!str_starts_with($href,rtrim($source->base_url,'/')))continue;
            if(!preg_match('/(job|recruit|vacancy|bharti|rojgar|notification|recruitment|2026|2025)/iu',$title.' '.$href))continue;
    ...
    private function parseDetail(string $html,string $url): array
    {
        ...
        $selectors=['//article','//*[contains(@class,"post-body")]','//*[contains(@class,"entry-content")]','//*[contains(@class,"post-content")]','//main'];
        foreach($selectors as $sel){$nodes=$xpath->query($sel);if($nodes&&$nodes->length){$content=trim($nodes->item(0)->textContent);if(mb_strlen($content)>100)break;}}
