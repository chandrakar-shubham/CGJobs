...
class JobSourceService
{
    public function sync(JobSource $source): array
    {
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                if (!$this->sameHost($url, $source->base_url)) { $skipped++; continue; }
                ...
                // HTML sources: fetch the individual article so we never publish a listing-page title as the article body.
                if (!in_array($source->source_type, ['rss','blogger','json_api','rest_api','wordpress'], true)) {
                    try { $item = array_merge($item, array_filter($this->parseDetail($this->get($url), $url), fn($v) => $v !== null && $v !== '')); }
                    catch (\Throwable $e) { Log::warning('Job detail fetch failed', ['url'=>$url,'error'=>$e->getMessage()]); }
                }
                ...
                // Source photos are not reused. The importer creates a distinct CGJobs cover for every URL.
                $image = $this->generateCover($title, $category, $url) ?: ($item['image_url'] ?? null);
    ...
    public function publish(JobImport $import): Job
    {
        ...
        $hasAiRewrite = trim((string)env('GEMINI_API_KEY','')) !== '' && ($rewritten['ai'] ?? false);
        ...
        $job = Job::create([
            'title'=>$titleHi,'title_en'=>$title,
            'summary'=>$summaryHi,'summary_en'=>$summary,
            'detailed_content'=>$contentHi,'detailed_content_en'=>$sourceContent,
            'category'=>$category,'category_en'=>$category,'section'=>'jobs','post_type'=>'job',
            'source'=>$import->source->name,'source_url'=>$import->external_url,'image_url'=>$cover,
            ...
            'application_start'=>'जारी','last_date'=>'शीघ्र','translation_status'=>$hasAiRewrite ? 'ready' : 'pending',
        ]);
        $import->update(['job_id'=>$job->id,'status'=>'published','processed_at'=>now()]);

        // If Gemini produced the Hindi rewrite, keep it. Otherwise use the existing free EN→HI translator.
        if (!$hasAiRewrite) {
            try {
                $t = app(TranslationService::class)->translateMany(['title'=>$title,'summary'=>$summary,'detailed'=>$sourceContent,'category'=>$category]);
                if (!empty($t['title'])) $job->update(['title'=>$t['title'],'summary'=>$t['summary'] ?: $summaryHi,'detailed_content'=>$t['detailed'] ?: $contentHi,'category'=>$t['category'] ?: $category,'translation_status'=>'ready','translation_error'=>null,'translated_at'=>now()]);
    ...
    private function rephrase(string $title,string $summary,string $content,string $category): array
    {
        ...
        if ($key === '') return ['title'=>$title,'summary'=>$this->makeShortSummary($summary),'content'=>$this->cleanText($content),'ai'=>false];
        try {
            $prompt = "You are the editorial engine for CGJobs, an Indian government-jobs information platform. Rewrite the supplied recruitment article in ORIGINAL wording. Do not copy sentences verbatim. Do not invent or change any fact, date, vacancy, salary, eligibility, name or link. Remove advertisements, promotional lines, navigation, social-media requests and repeated boilerplate. Produce natural, concise Hindi for Indian job seekers. Return JSON only with keys title, summary, content. summary must be 280-420 characters. content should be a clean article with useful headings and bullet-style lines in plain text.\n\nCategory: {$category}\nTITLE: {$title}\nSUMMARY: {$summary}\nSOURCE ARTICLE:\n{$content}";
            ...
            if (is_array($json) && !empty($json['content'])) return ['title'=>$this->cleanText($json['title'] ?? $title) ?: $title,'summary'=>$this->makeShortSummary($json['summary'] ?? $summary),'content'=>$this->cleanText($json['content']) ?: $content,'ai'=>true];
        } catch (\Throwable $e) { Log::warning('Gemini rephrase failed; using cleaned source',['error'=>$e->getMessage()]); }
        return ['title'=>$title,'summary'=>$this->makeShortSummary($summary),'content'=>$this->cleanText($content),'ai'=>false];
    }

    private function get(string $url): string
    { return Http::timeout(30)->retry(2,1000)->withHeaders(['User-Agent'=>'Mozilla/5.0 (compatible; CGJobsBot/1.0; +https://cgjobs.app)','Accept'=>'text/html,application/xhtml+xml,application/xml,application/json,text/plain;q=0.9,*/*;q=0.8'])->get($url)->throw()->body(); }
    ...
    private function parseRss(string $xml,JobSource $source): array
    {
        ...
        foreach($nodes as $n){$url=(string)($n->link['href']??$n->link);$title=$this->cleanText((string)$n->title);if(!$url||!$title)continue;$out[]=['url'=>$this->absoluteUrl($url,$source->base_url),'title'=>$title,'summary'=>$this->makeShortSummary((string)($n->description??$n->summary)),'content'=>$this->cleanText(strip_tags((string)($n->description??$n->summary))),'category'=>$this->detectCategory($title),'published_at'=>(string)($n->pubDate??$n->published)];}
    ...
    private function parseDetail(string $html,string $url): array
    {
        ...
        $title=$this->meta($xpath,'og:title')?:$this->firstText($xpath,'//h1')?:$this->firstText($xpath,'//title');$image=$this->meta($xpath,'og:image');$date=$this->meta($xpath,'article:published_time')?:$this->firstAttr($xpath,'//time[@datetime]','datetime');$node=null;
        ...
        $node=$node?:$dom->documentElement;$content=$this->nodeText($node);
        return ['title'=>$this->cleanText($title),'content'=>$content,'summary'=>$this->makeShortSummary($content),'image_url'=>$image,'published_at'=>$date,'url'=>$url];
    }
    ...
    { $parts=[];foreach($node->childNodes as $child){if($child->nodeType===XML_TEXT_NODE){$text=$this->cleanText($child->textContent);if($text!=='')$parts[]=$text;}elseif($child->nodeType===XML_ELEMENT_NODE){$text=$this->nodeText($child);if($text!=='')$parts[]=$text;}}return $this->cleanText(implode("\n",$parts)); }
    ...
    {
        try{$hash=substr(sha1($url),0,16);$dir=public_path('generated/jobs');if(!is_dir($dir))@mkdir($dir,0755,true);$path=$dir.'/job-'.$hash.'.svg';if(is_file($path))return'/generated/jobs/job-'.$hash.'.svg';$safeTitle=htmlspecialchars(Str::limit($title,54,'…'),ENT_QUOTES|ENT_XML1,'UTF-8');$safeCategory=htmlspecialchars(Str::limit($category,28,''),ENT_QUOTES|ENT_XML1,'UTF-8');$h=hexdec(substr($hash,0,6))%360;$h2=($h+48)%360;$svg='<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="675" viewBox="0 0 1200 675"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop stop-color="hsl('.$h.',72%,38%)"/><stop offset="1" stop-color="hsl('.$h2.',72%,58%)"/></linearGradient></defs><rect width="1200" height="675" rx="42" fill="url(#g)"/><circle cx="1040" cy="120" r="150" fill="white" opacity=".10"/><circle cx="110" cy="590" r="210" fill="white" opacity=".08"/><rect x="70" y="70" width="1060" height="535" rx="34" fill="white" opacity=".10"/><text x="100" y="155" font-family="Arial,sans-serif" font-size="30" font-weight="700" fill="white">CGJOBS • GOVERNMENT JOB UPDATE</text><text x="100" y="250" font-family="Arial,sans-serif" font-size="54" font-weight="700" fill="white">'.$safeTitle.'</text><text x="100" y="330" font-family="Arial,sans-serif" font-size="28" fill="white" opacity=".92">'.$safeCategory.'</text><rect x="100" y="430" width="230" height="72" rx="36" fill="white" opacity=".18"/><text x="215" y="476" text-anchor="middle" font-family="Arial,sans-serif" font-size="26" font-weight="700" fill="white">READ MORE →</text></svg>';file_put_contents($path,$svg);return'/generated/jobs/job-'.$hash.'.svg';}catch(\Throwable $e){Log::warning('Could not generate job cover',['error'=>$e->getMessage()]);return null;}
    }

    private function dom(string $html):\DOMDocument{$dom=new \DOMDocument();@$dom->loadHTML('<?xml encoding="UTF-8">'.$html);return$dom;}
    private function meta(\DOMXPath $xpath,string $property):?string{$nodes=$xpath->query('//meta[@property="'.$property.'"]/@content | //meta[@name="'.$property.'"]/@content');return($nodes&&$nodes->length)?trim((string)$nodes->item(0)->nodeValue):null;}
    private function firstText(\DOMXPath $xpath,string $selector):?string{$nodes=$xpath->query($selector);return($nodes&&$nodes->length)?$this->cleanText($nodes->item(0)->textContent):null;}
    private function firstAttr(\DOMXPath $xpath,string $selector,string $attr):?string{$nodes=$xpath->query($selector);return($nodes&&$nodes->length)?trim((string)$nodes->item(0)->getAttribute($attr)):null;}
    private function absoluteUrl(string $url,string $base):string{$url=trim($url);if($url==='')return'';if(str_starts_with($url,'//'))return'https:'.$url;if(filter_var($url,FILTER_VALIDATE_URL))return$url;return rtrim($base,'/').'/'.ltrim($url,'/');}
