...
class JobSourceService
{
                ...
                if ($source->source_type === 'html') {
                    try {
                        $item = array_merge($item, array_filter(
                            $this->parseDetail($this->get($url), $url),
                            fn ($v) => $v !== null && $v !== ''
                        ));
                    } catch (\Throwable $e) {
    ...
    public function publish(JobImport $import): Job
    {
        ...
        try {
            ...
            $job->update([
                ...
                'translation_status' => !empty($t['title']) ? 'ready' : 'pending',
                'translation_error' => empty($t['title']) ? 'Automatic Hindi translation temporarily unavailable.' : null,
                'translated_at' => !empty($t['title']) ? now() : null,
            ]);
        } catch (\Throwable $e) {
    ...
    private function detectMainCategory(string $text, ?string $default): string
    {
        ...
        if (in_array($default, self::MAIN_CATEGORIES, true)) return $default;
    ...
    private function parseDetail(string $html, string $url): array
    {
        ...
        return ['title' => $this->cleanText($title), 'content' => $this->nodeText($node), 'summary' => $this->makeShortSummary($this->nodeText($node)), 'published_at' => $date, 'url' => $url];
    }

    private function dom(string $html): \DOMDocument
    {
        libxml_use_internal_errors(true); $dom = new \DOMDocument(); $dom->loadHTML('<?xml encoding="UTF-8">'.$html, LIBXML_NOWARNING | LIBXML_NOERROR); libxml_clear_errors(); return $dom;
    }
    ...
    private function nodeText(\DOMNode $node): string
    {
        ...
        foreach ($node->childNodes as $child) {
            if ($child->nodeType === XML_TEXT_NODE) { $text = $this->cleanText($child->textContent); if ($text !== '') $parts[] = $text; }
            elseif ($child->nodeType === XML_ELEMENT_NODE) { $text = $this->nodeText($child); if ($text !== '') $parts[] = $text; }
        }
        return $this->cleanText(implode("\n", $parts));
    }

    private function meta(\DOMXPath $xpath, string $property): ?string
    { $n = $xpath->query('//meta[@property="'.$property.'"]|//meta[@name="'.$property.'"]')->item(0); return $n ? trim((string) $n->getAttribute('content')) : null; }
