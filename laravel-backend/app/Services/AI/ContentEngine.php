...
class ContentEngine
{
    public function process(array $items): array
    {
        ...
        $requestCount = (int) AiUsageLog::where('provider_setting_id', $setting->id)
            ->whereDate('created_at', today())->count();
        $tokenCount = (int) AiUsageLog::where('provider_setting_id', $setting->id)
            ->whereDate('created_at', today())->sum('input_tokens')
            + (int) AiUsageLog::where('provider_setting_id', $setting->id)
            ->whereDate('created_at', today())->sum('output_tokens');

        // Keep batches deliberately small. Gemini structured JSON can become a large
        // response when each item contains mobile + website + SEO content.
        $maxItems = $this->effectiveBatchSize($setting);
        $saved = [];

        foreach (array_chunk($items, $maxItems) as $chunk) {
            if ($requestCount >= (int) $setting->daily_request_limit) {
                foreach ($chunk as $item) {
                    $this->markFailed($item, 'Daily AI request limit reached.');
                }
                continue;
            }
                ...
                if (count($chunk) > 1) {
                    foreach ($this->splitByTokenBudget($chunk, max(1, (int) $setting->daily_token_limit - $tokenCount)) as $smaller) {
                        ...
                        if ($tokenCount + $this->estimateTokens($smaller) > (int) $setting->daily_token_limit) {
                            foreach ($smaller as $item) {
                                $this->markFailed($item, 'Daily AI token limit reached.');
                            }
                            continue;
                        }
                    ...
                    $this->markFailed($chunk[0], 'Daily AI token limit reached.');
                }
                continue;
            }
    ...
    public function buildJob(Job $job): array
    {
        return [
            ...
            'source' => [
                ...
                'detailed_content' => $job->detailed_content,
                'detailed_content_en' => $job->detailed_content_en,
                'category' => $job->category,
                'department' => $job->department,
                'vacancies' => $job->vacancies,
                'salary' => $job->salary,
                'eligibility' => $job->eligibility,
                'eligibility_en' => $job->eligibility_en,
                'age_limit' => $job->age_limit,
                'selection_process' => $job->selection_process,
                'selection_process_en' => $job->selection_process_en,
    ...
    private function effectiveBatchSize(AiProviderSetting $setting): int
    {
        $configured = max(1, min((int) $setting->max_items_per_request, 100));
        $provider = strtolower((string) $setting->provider);
        return $provider === 'gemini' ? min($configured, 10) : min($configured, 20);
    }
    ...
    private function processBatch(AiProviderSetting $setting, array $items): array
    {
        try {
            ...
            return [
                'saved' => $this->saveResults($items, $response, $setting),
                'tokens' => $this->responseTokens($response),
                'requests' => 1,
            ];
            ...
            if ($fallback && isset($fallbackError)) {
                $message = $fallbackError->getMessage();
            }
            foreach ($items as $item) {
                $this->markFailed($item, 'AI generation failed: ' . substr($message, 0, 700));
            }
    ...
    private function saveResults(array $items, array $response, AiProviderSetting $setting): array
    {
        ...
        if (!is_array($results)) {
            throw new RuntimeException('AI response has no results array.');
        }

        $byId = [];
        foreach ($results as $result) {
            if (is_array($result) && isset($result['id'])) {
                $byId[(int) $result['id']] = $result;
            }
        }
        if (!$byId) {
            throw new RuntimeException('AI response contained no valid item results.');
        }
    ...
    {
        if ((int) ($result['id'] ?? 0) !== (int) $item['id']) {
            throw new RuntimeException('AI result id mismatch.');
        }
        foreach (['mobile', 'website', 'seo'] as $section) {
            if (!isset($result[$section]) || !is_array($result[$section])) {
                throw new RuntimeException('AI result missing ' . $section . ' section.');
            }
        }
        if (!is_string($result['mobile']['summary'] ?? null) || trim($result['mobile']['summary']) === '') {
            throw new RuntimeException('AI result missing mobile summary.');
        }
        if (!is_string($result['website']['content'] ?? null) || trim($result['website']['content']) === '') {
            throw new RuntimeException('AI result missing website content.');
        }
        if (!is_string($result['seo']['title'] ?? null) || !is_string($result['seo']['description'] ?? null)) {
            throw new RuntimeException('AI result missing SEO fields.');
        }
    }
    ...
    private function markFailed(array $item, string $message): void
    {
        AiContent::updateOrCreate(
            ['source_type' => $item['source_type'], 'source_id' => $item['id']],
            [
                'status' => 'failed',
                'source_snapshot' => $item['source'],
                'attempts' => 1,
    ...
    private function callProvider(AiProviderSetting $setting, array $items, bool $fallbackUsed = false): array
    {
        ...
        if ($provider === 'gemini') {
            ...
            $r = Http::timeout(120)
                ...
                ->post($url, [
                    ...
                    // REST generateContent uses snake_case in the JSON-mode example.
                    'generationConfig' => [
            ...
            $r = Http::timeout(120)->withToken($setting->api_key)->post(
                ...
                [
                    ...
                    'messages' => [
                        ['role' => 'system', 'content' => 'Return valid JSON only. Never invent or modify factual job/news details.'],
        ...
        if (!$r->successful()) {
            ...
            $detail = is_array($error)
                ? trim((string) ($error['message'] ?? json_encode($error, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)))
                : trim($r->body());
            $detail = $detail !== '' ? ' ' . substr($detail, 0, 700) : '';
            throw new RuntimeException('AI provider request failed with HTTP ' . $r->status() . '.' . $detail);
        }
    ...
    private function estimateTokens(array $items): int
    {
        return max(1, (int) ceil(strlen($this->prompt($items)) / 4)) + (count($items) * 700);
    }

    private function splitByTokenBudget(array $items, int $budget): array
    {
        $chunks = [];
        $current = [];
        foreach ($items as $item) {
            $candidate = array_merge($current, [$item]);
            if ($current && $this->estimateTokens($candidate) > $budget) {
                $chunks[] = $current;
                $current = [$item];
            } else {
                $current = $candidate;
...
PROMPT;
    ...
    private function applyGeneratedContent(array $item, array $result, AiProviderSetting $setting): void
    {
        if ($item['source_type'] === 'news') {
            ...
            $mobile = $result['mobile'] ?? [];
            $job->update([
                'summary' => $mobile['summary'] ?? $job->summary,
