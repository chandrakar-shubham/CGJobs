...
class NewsRssFallback
{
        ...
        foreach ($feedUrls as $feedUrl) {
            ...
            try {
                ...
                foreach ($xml->channel->item as $item) {
                    ...
                    if ($rawTitle === '' || $this->isGenericFeedItem($rawTitle, $url)) {
                        continue;
                    }
                    ...
                    if ($this->isGenericDescription($description) && $contentEncoded !== '') {
                    ...
                    if ($strictGeo && !$this->passesGeographyFilter($title . ' ' . $description, $geo, $topic)) {
                        continue;
                    }
                    ...
                    if (isset($seenTitles[$titleKey]) || ($urlKey !== '' && isset($seenUrls[$urlKey]))) {
                        continue;
                    }

                    $enriched = $this->enrichArticle($url, $title, $description, $image);
                    $finalTitle = $enriched['title'] ?: $title;
                    $finalDescription = $enriched['description'] ?: $description;
                    $finalUrl = $enriched['url'] ?: $url;
                    $finalImage = $this->usableImageUrl($enriched['image'] ?: $image);

                    if ($this->isGenericMetadata($finalTitle, $finalDescription, $finalUrl)) {
                        $finalTitle = $title;
                    ...
                    $finalDescription = $this->cleanText($finalDescription);
                    if ($finalDescription === '' || $this->isGenericDescription($finalDescription) || $finalDescription === $finalTitle) {
                        $finalDescription = $this->extractArticleDescription($enriched['_html'] ?? '') ?: $description;
                    }
                    $finalDescription = $this->cleanText($finalDescription);
                    if ($finalDescription === '' || $finalDescription === $finalTitle) {
                        // Never replace a missing article description with a Google News boilerplate string.
                    ...
                    if ($finalTitle === '' || $this->isGenericTitle($finalTitle)) {
                        continue;
                    }
                    if (isset($seenTitles[$finalTitleKey]) || ($finalUrlKey !== '' && isset($seenUrls[$finalUrlKey]))) {
                        continue;
                    }
    ...
    private function enrichArticle(string $url, string $fallbackTitle, ?string $fallbackDescription, ?string $fallbackImage): array
    {
        ...
        try {
            ...
            $title = $this->metaValue($html, 'property', 'og:title')
                ?: $this->metaValue($html, 'name', 'twitter:title')
                ?: $fallbackTitle;
            $description = $this->metaValue($html, 'property', 'og:description')
                ?: $this->metaValue($html, 'name', 'description')
                ?: $this->metaValue($html, 'name', 'twitter:description')
                ?: $fallbackDescription;
            $image = $this->metaValue($html, 'property', 'og:image')
                ?: $this->metaValue($html, 'name', 'twitter:image')
                ?: $this->metaValue($html, 'itemprop', 'image')
                ?: $fallbackImage;
            ...
            if ($finalUrl === '') {
                ...
                if ($effective !== '' && !$this->isGoogleNewsUrl($effective)) {
                    $finalUrl = $effective;
                }
            }

            $jsonLd = $this->jsonLdArticle($html);
            if (($description === '' || $this->isGenericDescription($description)) && !empty($jsonLd['description'])) {
                $description = (string) $jsonLd['description'];
            }
            if (($image === '' || !$this->usableImageUrl($image)) && !empty($jsonLd['image'])) {
                $image = is_array($jsonLd['image'])
                    ? (string) ($jsonLd['image'][0] ?? '')
                    : (string) $jsonLd['image'];
            }
            if (($title === '' || $this->isGenericTitle($title)) && !empty($jsonLd['headline'])) {
                $title = (string) $jsonLd['headline'];
            }

            if (!$this->usableImageUrl($image)) {
                $image = $this->extractArticleImage($html);
            }

            if ($description === '' || $this->isGenericDescription($description)) {
                $description = $this->extractArticleDescription($html) ?: $fallbackDescription;
            }
    ...
    private function extractArticleImage(string $html): ?string
    {
        if ($html === '') {
            return null;
        }
        ...
        if (preg_match_all('/<img[^>]+(?:src|data-src|data-original)=["\']([^"\']+)["\'][^>]*>/i', $html, $matches)) {
            foreach ($matches[1] as $candidate) {
                ...
                $candidate = $this->absoluteUrl($candidate, 'https://example.com/');
                if ($this->usableImageUrl($candidate)) {
                    return $candidate;
                }
            }
        }
    ...
    private function extractArticleDescription(string $html): ?string
    {
        ...
        if (preg_match('/<(?:article|main)[^>]*>(.*?)<\/(?:article|main)>/is', $html, $match)) {
            $text = $this->cleanText(strip_tags($match[1], '<p>'));
            if ($text !== '' && Str::length($text) >= 80 && !$this->isGenericDescription($text)) {
        ...
        if (preg_match_all('/<p[^>]*>(.*?)<\/p>/is', $html, $matches)) {
            ...
            foreach ($matches[1] as $paragraph) {
                ...
                if (Str::length($text) >= 50 && !$this->isGenericDescription($text)) {
                    $paragraphs[] = $text;
                }
                if (Str::length(implode(' ', $paragraphs)) >= 500) {
                    break;
                }
            }
            if ($paragraphs) {
                return Str::limit(implode(' ', $paragraphs), 500, '');
            }
        }
    ...
    private function isGenericTitle(?string $value): bool
    {
        ...
        return in_array($v, ['google news', 'google', 'news', 'google trends', 'trending searches', 'home'], true)
            || Str::length($v) < 12;
    }
    ...
    private function isGenericMetadata(?string $title, ?string $description, ?string $url): bool
    {
        return $this->isGenericTitle($title)
            || $this->isGenericDescription($description)
            || $this->isGoogleNewsUrl($url);
    }
    ...
    private function metaValue(string $html, string $attribute, string $value): string
    {
        ...
        return preg_match($pattern, $html, $m)
    ...
    private function jsonLdArticle(string $html): array
    {
        ...
        foreach ($matches[1] as $raw) {
            ...
            foreach ($nodes as $node) {
                ...
                if (Str::contains(Str::lower((string) $type), ['newsarticle', 'article', 'reportagenewsarticle'])) {
                    return $node;
                }
            }
        }
