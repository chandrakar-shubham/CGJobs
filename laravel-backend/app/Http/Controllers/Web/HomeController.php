...
class HomeController extends Controller
{
    ...
    public function listing(string $type, Request $request): View
    {
        ...
        } elseif($category!=='') $query->where(fn($q)=>$q->where('category',$category)->orWhere('category_en',$category)->orWhere('category_hindi',$category));
        ...
        return view('web.listing',['type'=>$type,'title'=>$lang==='en'?$map[$type]['title_en']:$map[$type]['title_hi'],'items'=>$items,'categories'=>$categories,'search'=>$search,'selectedCategory'=>$category,'selectedJobCategory'=>$jobCategory,'selectedDepartment'=>$department,'jobCategories'=>self::JOB_CATEGORIES,'departments'=>self::DEPARTMENTS,'sort'=>$sort,'lang'=>$lang]);
    }
    ...
    private function language(Request $request):string{$value=$request->query('lang',session('locale','hi'));return in_array($value,['hi','en'],true)?$value:'hi';}
