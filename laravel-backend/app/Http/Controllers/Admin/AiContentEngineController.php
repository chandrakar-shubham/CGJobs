...
class AiContentEngineController extends Controller
{
    ...
    {
        $data=$request->validate(['provider'=>'required|in:gemini,groq','api_key'=>'nullable|string|max:500','model'=>'nullable|string|max:120','fallback_provider'=>'nullable|in:gemini,groq','fallback_api_key'=>'nullable|string|max:500','fallback_model'=>'nullable|string|max:120','enabled'=>'nullable|boolean','daily_request_limit'=>'required|integer|min:1|max:100000','daily_token_limit'=>'required|integer|min:1|max:100000000','max_items_per_request'=>'required|integer|min:1|max:100','monthly_budget'=>'nullable|numeric|min:0','auto_publish_news'=>'nullable|boolean','auto_publish_jobs'=>'nullable|boolean']);
        $setting=AiProviderSetting::latest()->first()?:new AiProviderSetting();
        if(!empty($data['api_key']))$setting->api_key=$data['api_key']; if(!empty($data['fallback_api_key']))$setting->fallback_api_key=$data['fallback_api_key']; unset($data['api_key'],$data['fallback_api_key']);
        $setting->fill($data);$setting->enabled=$request->boolean('enabled');$setting->auto_publish_news=$request->boolean('auto_publish_news');$setting->auto_publish_jobs=$request->boolean('auto_publish_jobs');$setting->save();
    ...
    public function ingestNews(Request $request, NewsIngestionService $ingestion)
    {
        ...
        try{$r=$ingestion->ingestAndProcess($data['query']??null,(int)($data['limit']??20),true);}catch(\Throwable$e){return back()->withErrors(['ai'=>'News pipeline failed: '.$e->getMessage()]);}
        return back()->with('success',"News pipeline: fetched {$r['fetched']}, created {$r['created']}, AI processed {$r['processed']}.");
    }
    ...
    public function preview(AiContent $aiContent){abort_unless($aiContent->generated_content,404);return view('admin.ai-engine.preview',['content'=>$aiContent]);}
    public function retry(AiContent $aiContent,ContentEngine $engine){try{$engine->process([['id'=>$aiContent->source_id,'source_type'=>$aiContent->source_type,'source'=>$aiContent->source_snapshot]]);}catch(\Throwable$e){return back()->withErrors(['ai'=>$e->getMessage()]);}return back()->with('success','Content regenerated.');}
    public function publish(AiContent $aiContent){if($aiContent->source_type==='news'){$news=News::findOrFail($aiContent->source_id);$news->update(['status'=>'published','published_at'=>$news->published_at?:now()]);}else{$job=Job::findOrFail($aiContent->source_id);$job->update(['workflow_status'=>'published']);}$aiContent->update(['status'=>'published','published_at'=>now()]);return back()->with('success','Content published.');}
}
