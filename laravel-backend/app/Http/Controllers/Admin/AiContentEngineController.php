...
class AiContentEngineController extends Controller
{
    ...
    public function settings(Request $request)
    {
        $data = $request->validate([
            ...
            'daily_token_limit'=>'required|integer|min:1|max:100000000',
            'max_items_per_request'=>'required|integer|min:1|max:100',
            'monthly_budget'=>'nullable|numeric|min:0',
            'auto_publish_news'=>'nullable|boolean',
            'auto_publish_jobs'=>'nullable|boolean',
        ]);
        $setting = AiProviderSetting::latest()->first();
        if (!$setting) $setting = new AiProviderSetting();
    ...
    public function process(Request $request, ContentEngine $engine)
    {
        ...
        $items = collect($data['ids'])->map(function ($id) use ($data, $engine) {
            if ($data['type'] === 'news') {
                $m = News::find($id); return $m ? $engine->buildNews($m) : null;
            }
            $m = Job::find($id); return $m ? $engine->buildJob($m) : null;
        })->filter()->values()->all();
        try { $saved = $engine->process($items); }
        catch (\Throwable $e) { return back()->withErrors(['ai'=>$e->getMessage()]); }
        return back()->with('success', count($saved).' item(s) generated.');
    }
    ...
    public function retry(AiContent $aiContent, ContentEngine $engine)
    {
        try {
            $items = [[
                'id'=>$aiContent->source_id,
                'source_type'=>$aiContent->source_type,
                'source'=>$aiContent->source_snapshot,
            ]];
    ...
    {
        $record = $aiContent->generated_content ?: [];
