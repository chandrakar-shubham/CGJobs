...
class JobSourceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.job-sources.index',['sources'=>JobSource::latest()->get(),'pending'=>$pending,'published'=>$published,'filters'=>$filters,'mainCategories'=>self::MAIN_CATEGORIES,'departments'=>self::DEPARTMENTS]);
    }

    public function store(Request $request){$data=$request->validate(['name'=>'required|string|max:120','base_url'=>'required|url|max:500','fetch_url'=>'required|url|max:1000','source_type'=>'required|in:html,rss,blogger,wordpress,json_api,rest_api','frequency_minutes'=>'required|integer|min:15|max:43200','publish_mode'=>'required|in:approval,auto','default_category'=>'required|in:CGSSB,CGPSC,Central Govt,Contractual']);$data['is_active']=$request->boolean('is_active');JobSource::create($data);return back()->with('success','Job source added successfully.');}
    public function update(Request $request,JobSource $jobSource){$data=$request->validate(['name'=>'required|string|max:120','base_url'=>'required|url|max:500','fetch_url'=>'required|url|max:1000','source_type'=>'required|in:html,rss,blogger,wordpress,json_api,rest_api','frequency_minutes'=>'required|integer|min:15|max:43200','publish_mode'=>'required|in:approval,auto','default_category'=>'required|in:CGSSB,CGPSC,Central Govt,Contractual']);$data['is_active']=$request->boolean('is_active');$jobSource->update($data);return back()->with('success','Job source updated.');}
    public function destroy(JobSource $jobSource){$jobSource->delete();return back()->with('success','Job source removed.');}

    public function sync(Request $request,JobSource $jobSource,JobSourceService $service,JobImportNormalizer $normalizer){try{$from=$request->filled('sync_from')?$request->date('sync_from'):null;$to=$request->filled('sync_to')?$request->date('sync_to'):null;$result=$service->sync($jobSource,$from,$to,true);JobImport::where('job_source_id',$jobSource->id)->where('status','pending')->latest('id')->limit(max(1,(int)$result['imported']))->get()->each(fn($import)=>$normalizer->normalize($import));return back()->with('success',"Sync complete: {$result['fetched']} fetched, {$result['imported']} imported, {$result['skipped']} skipped. Titles and source branding normalized.");}catch(\Throwable $e){report($e);return back()->withErrors(['sync'=>'Sync failed: '.$e->getMessage()]);}}
    ...
    public function updateImport(Request $request,JobImport $jobImport){$data=$request->validate(['title'=>'required|string|max:250','summary'=>'nullable|string|max:2000','content'=>'nullable|string','job_category'=>'required|in:CGSSB,CGPSC,Central Govt,Contractual','department'=>'required|string|max:120','external_url'=>'required|url|max:2000','apply_url'=>'nullable|url|max:2000','notification_url'=>'nullable|url|max:2000','image_url'=>'nullable|url|max:2000','published_at'=>'nullable|date']);$payload=is_array($jobImport->raw_payload)?$jobImport->raw_payload:[];$facts=is_array($payload['facts']??null)?$payload['facts']:[];$facts['apply_url']=$data['apply_url']??null;$facts['notification_url']=$data['notification_url']??null;$payload['facts']=$facts;$payload['edited_in_admin']=true;$payload['edited_at']=now()->toIso8601String();$jobImport->update(['title'=>$data['title'],'summary'=>$data['summary']??null,'content'=>$data['content']??null,'category'=>$data['department'],'job_category'=>$data['job_category'],'department'=>$data['department'],'published_by'=>$data['job_category'],'external_url'=>$data['external_url'],'image_url'=>$data['image_url']??null,'published_at'=>$data['published_at']??null,'raw_payload'=>$payload]);if($jobImport->status==='published'&&$jobImport->job)$this->syncPublishedJob($jobImport->fresh('job'));return redirect()->route('admin.job-sources.index',$request->only(['q','source_id','job_category','from','to']))->with('success','Imported job data updated successfully.');}

    public function approve(JobImport $jobImport,JobSourceService $service,JobImportNormalizer $normalizer){$normalizer->normalize($jobImport);$job=$service->publish($jobImport->fresh());$job->update(['published_by'=>$job->published_by?:$job->job_category?:$jobImport->job_category?:'CGSSB']);return back()->with('success','Approved and published. View webpage: '.route('job.show',$job->custom_id?:$job->id));}

    public function approveAll(Request $request,JobSourceService $service,JobImportNormalizer $normalizer){$filters=$this->filters($request);$query=JobImport::where('status','pending')->when($filters['q'],fn($q)=>$q->where(fn($w)=>$w->where('title','like','%'.$filters['q'].'%')->orWhere('external_url','like','%'.$filters['q'].'%')))->when($filters['source_id'],fn($q)=>$q->where('job_source_id',$filters['source_id']))->when($filters['job_category'],fn($q)=>$q->where('job_category',$filters['job_category']))->when($filters['from'],fn($q)=>$q->whereDate('published_at','>=',$filters['from']))->when($filters['to'],fn($q)=>$q->whereDate('published_at','<=',$filters['to']));$count=0;$query->chunkById(100,function($imports)use($service,$normalizer,&$count){foreach($imports as $import){$normalizer->normalize($import);$service->publish($import->fresh());$count++;}});return back()->with('success',"{$count} imported jobs approved and published.");}
    ...
    private function filters(Request $request):array{return['q'=>trim((string)$request->query('q','')),'source_id'=>$request->query('source_id'),'job_category'=>$request->query('job_category'),'from'=>$request->query('from'),'to'=>$request->query('to')];}
