...
class NewsController extends Controller
{
    private const CATEGORIES = [
        ...
        'Reports & Indexes', 'Important Days', 'Sports',
    ];
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $query->where(fn ($q) => $q->where('title', 'like', $term)->orWhere('title_en', 'like', $term)->orWhere('summary', 'like', $term)->orWhere('summary_en', 'like', $term)->orWhere('source', 'like', $term));
        }
        foreach (['category', 'status', 'language'] as $filter) {
            if ($request->filled($filter)) $query->where($filter, $request->$filter);
        }
        if ($request->filled('featured')) $query->where('featured', (bool) $request->featured);
        $news = $query->orderByDesc('published_at')->orderByDesc('id')->paginate(20)->withQueryString();
        return view('admin.news.index', ['news' => $news, 'categories' => self::CATEGORIES]);
    }
    ...
    public function store(Request $request)
    {
        ...
        News::create($data);
        return redirect()->route('admin.news.index')->with('success', 'Current affairs article created.');
    }
    ...
    public function update(Request $request, News $news)
    {
        ...
        $data['tags'] = $this->tags($request->input('tags'));
        $news->update($data);
        return redirect()->route('admin.news.index')->with('success', 'Current affairs article updated.');
    }

    public function publish(News $news)
    {
        $news->update(['status' => 'published', 'published_at' => $news->published_at ?: now()]);
        return back()->with('success', 'Current affairs article published.');
    }

    public function archive(News $news)
    {
        $news->update(['status' => 'archived']);
        return back()->with('success', 'Current affairs article archived.');
    }

    public function destroy(News $news)
    {
        $news->delete();
        return back()->with('success', 'Current affairs article deleted.');
    }
    ...
    private function validated(Request $request): array
    {
        return $request->validate([
            ...
            'exam_relevance' => 'nullable|integer|min:0|max:5', 'status' => 'required|in:draft,review,published,archived',
