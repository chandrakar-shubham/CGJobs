document.addEventListener('DOMContentLoaded',function(){
 ...
 const title=q('[data-seo="title"]'), summary=q('[data-seo="summary"]'), content=q('[data-seo="content"]'), image=q('[data-seo="image"]'), apply=q('[data-seo="apply"]'), note=q('[name="official_notification_url"]'), source=q('[name="source_url"]');
 ...
 function run(){if(!checks)return;let rows=[],points=0;const t=(title?.value||'').trim(),s=(summary?.value||'').trim(),c=(content?.value||'').trim();
  ...
  add('Full description',c.length>=300,c.length+' chars','Add complete recruitment details');
  ...
  const tips=[];if(t.length<30||t.length>70)tips.push('Keep Job Title between 30–70 characters.');if(s.length<50||s.length>160)tips.push('Keep Short Summary between 50–160 characters.');if(c.length<300)tips.push('Add enough full description for useful search content.');if(!image?.value)tips.push('Add a relevant Main Image.');if(!apply?.value)tips.push('Add the official Apply URL.');suggest.innerHTML='<b>Suggestions</b><br>'+((tips.length?tips:['Content is well prepared for publishing.']).map(x=>'• '+x).join('<br>'));
 ...
 const w=q('#cjWorkflow'),sf=q('#cjSchedule');function sync(){if(w&&sf)sf.style.display=w.value==='scheduled'?'block':'none'}w?.addEventListener('change',sync);sync();run();
});
