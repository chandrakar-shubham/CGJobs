...
class NewsIngestionService
{
    ...
    public function ingestAndProcess(?string $query = null, int $limit = 20, bool $process = true, array $filters = []): array
    {
        ...
        $articles = $this->fetch($query, $limit, $filters);
        $created = [];
    ...
    private function fetch(string $query, int $limit, array $filters = []): array
    {
        ...
        $to = $filters['to'] ?? null;
        $location = ['chhattisgarh' => 'Chhattisgarh', 'india' => 'India', 'world' => 'world international'][$geography] ?? 'Chhattisgarh';

        if (in_array($source, ['all', 'newsdata'], true) && ($key = config('services.newsdata.key'))) {
            try {
                $params = [
                    'apikey' => $key,
                    'q' => trim($query . ($topic ? ' ' . $topic : '')),
                    'language' => 'en',
                    'size' => min($limit, 10),
                ];
                if ($geography === 'india') $params['country'] = 'in';
                if ($from) $params['from_date'] = substr((string) $from, 0, 10);
                if ($to) $params['to_date'] = substr((string) $to, 0, 10);
        ...
        if (count($out) < $limit && in_array($source, ['all', 'newsapi'], true) && ($key = config('services.newsapi.key'))) {
            try {
                ...
                if ($to) $params['to'] = $to;
                if ($geography === 'india') $params['domains'] = $params['domains'] ?? null;
                $r = Http::timeout(20)->get('https://newsapi.org/v2/everything', array_filter($params, fn ($v) => $v !== null && $v !== ''));
                if ($r->successful()) {
        ...
        if (count($out) < $limit && in_array($source, ['all', 'google_news', 'google_trending'], true)) {
            $rssFilters = $filters;
            $rssFilters['query'] = $query;
            $rssFilters['strict_geo'] = $filters['strict_geo'] ?? false;
            $rssFilters['from'] = $from;
            $rssFilters['to'] = $to;
            $rss = app(NewsRssFallback::class)->fetch($query, $limit - count($out), $rssFilters);
