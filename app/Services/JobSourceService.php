...
class JobSourceService
{
    ...
    private const DEPARTMENTS = [
        'Education'=>['education','teacher','shikshak','school','vyakhyata','lecturer','professor','school education','samagra shiksha'],
    ...
    public function sync(JobSource $source): array
    {
        ...
        try {
            ...
            $items=match($source->source_type){'rss','blogger'=>$this->parseRss($raw,$source),'json_api','rest_api','wordpress'=>$this->parseJson($raw,$source),default=>$this->parseListing($raw,$source)};
            $imported=$skipped=0;
            foreach($items as $item){
                ...
                if($source->source_type==='html')try{$item=array_merge($item,array_filter($this->parseDetail($this->get($url),$url),fn($v)=>$v!==null&&$v!==''));}catch(\Throwable $e){Log::warning('Job detail fetch failed',['url'=>$url,'error'=>$e->getMessage()]);}
                $title=$this->cleanText($item['title']??'');$content=$this->cleanText($item['content']??'');$summary=$this->makeShortSummary($item['summary']??($content?:$title));$combined=mb_strtolower($title.' '.$summary.' '.$content);
                $jobCategory=$this->detectMainCategory($combined,$source->default_category);$department=$this->detectDepartment($combined,$item['category']??null);$facts=$this->extractFacts($combined);
                $import=JobImport::create(['job_source_id'=>$source->id,'external_key'=>sha1($url),'external_url'=>$url,'title'=>Str::limit($title,250,''),'summary'=>$summary,'content'=>$content?:$summary,'category'=>$department,'job_category'=>$jobCategory,'department'=>$department,'image_url'=>null,'published_at'=>$item['published_at']??null,'raw_payload'=>array_merge($item,['facts'=>$facts]),'status'=>$source->publish_mode==='auto'?'approved':'pending','fetched_at'=>now()]);
        ...
        }catch(\Throwable $e){$source->update(['last_error'=>Str::limit($e->getMessage(),1000,'')]);Log::error('Job source sync failed',['source'=>$source->id,'error'=>$e->getMessage()]);throw $e;}
    }

    public function publish(JobImport $import): Job
    {
        if($import->job_id)return $import->job;
        $existing=Job::where('source_url',$import->external_url)->first();if($existing){$import->update(['job_id'=>$existing->id,'status'=>'published','processed_at'=>now()]);return $existing;}
        $title=trim($import->title);$content=trim($import->content?:$import->summary?:$title);$summary=$this->makeShortSummary($import->summary?:$content);$jobCategory=in_array($import->job_category,self::MAIN_CATEGORIES,true)?$import->job_category:$this->detectMainCategory($title.' '.$content,null);$department=$import->department?:$this->detectDepartment($title.' '.$content,$import->category);$facts=is_array($import->raw_payload['facts']??null)?$import->raw_payload['facts']:$this->extractFacts($title.' '.$content);
        $job=Job::create(['title'=>$title,'title_en'=>$title,'summary'=>$summary,'summary_en'=>$summary,'detailed_content'=>$content,'detailed_content_en'=>$content,'category'=>$department,'category_en'=>$department,'job_category'=>$jobCategory,'department'=>$department,'section'=>'jobs','post_type'=>'job','source'=>$import->source?->name?:'Source Website','source_url'=>$import->external_url,'image_url'=>null,'published_at'=>optional($import->published_at)->format('Y-m-d')?:now()->format('Y-m-d'),'relative_time'=>'हाल ही में','relative_time_en'=>'Recently','is_new'=>true,'vacancies'=>$facts['vacancies']??null,'application_start'=>$facts['application_start']??'जारी','last_date'=>$facts['last_date']??'शीघ्र','apply_url'=>$facts['apply_url']??null,'official_notification_url'=>$facts['notification_url']??null,'translation_status'=>'pending']);
        $import->update(['job_id'=>$job->id,'status'=>'published','processed_at'=>now()]);
        try{$t=app(TranslationService::class)->translateMany(['title'=>$job->title_en,'summary'=>$job->summary_en,'detailed'=>$job->detailed_content_en,'category'=>$job->category_en]);$job->update(['title'=>$t['title']?:$job->title,'summary'=>$t['summary']?:$job->summary,'detailed_content'=>$t['detailed']?:$job->detailed_content,'category'=>$t['category']?:$job->category,'translation_status'=>!empty($t['title'])?'ready':'pending','translation_error'=>empty($t['title'])?'Automatic Hindi translation temporarily unavailable.':null,'translated_at'=>!empty($t['title'])?now():null]);}catch(\Throwable $e){Log::warning('Imported job translation failed',['job'=>$job->id,'error'=>$e->getMessage()]);}
        return $job->fresh();
    }

    private function detectMainCategory(string $text,?string $default): string
    {
        $text=mb_strtolower($text);$default=trim((string)$default);if(in_array($default,self::MAIN_CATEGORIES,true))return $default;
    ...
    {
        $haystack=mb_strtolower($text.' '.(string)$hint);foreach(self::DEPARTMENTS as $department=>$keywords)foreach($keywords as $keyword)if($keyword!==''&&str_contains($haystack,mb_strtolower($keyword)))return $department);return 'Other Departments';
    }
    ...
    {
        $dom=$this->dom($html);$xpath=new \DOMXPath($dom);$out=[];foreach($xpath->query('//a[@href]') as $a){$title=$this->cleanText($a->textContent);$href=$this->absoluteUrl(trim($a->getAttribute('href')),$source->base_url);if(!$href||mb_strlen($title)<12||!$this->sameHost($href,$source->base_url))continue;if(!preg_match('/(job|recruit|vacancy|bharti|rojgar|notification|recruitment|2026|2025|teacher|police|admit)/iu',$title.' '.$href))continue;$out[$href]=['url'=>$href,'title'=>$title,'summary'=>$title,'category'=>$this->detectDepartment($title)];}return array_slice(array_values($out),0,30);
    }

    private function parseRss(string $xml,JobSource $source): array
    {
        $simple=@simplexml_load_string($xml);if(!$simple)return[];$nodes=$simple->channel->item??$simple->entry??[];$out=[];foreach($nodes as $n){$url=(string)($n->link['href']??$n->link);$title=$this->cleanText((string)$n->title);if(!$url||!$title)continue;$out[]=['url'=>$this->absoluteUrl($url,$source->base_url),'title'=>$title,'summary'=>$this->makeShortSummary((string)($n->description??$n->summary)),'content'=>$this->cleanText(strip_tags((string)($n->description??$n->summary))),'category'=>$this->detectDepartment($title),'published_at'=>(string)($n->pubDate??$n->published)];}return array_slice($out,0,30);
    }

    private function parseJson(string $json,JobSource $source): array
    {
        $data=json_decode($json,true);if(!is_array($data))return[];$rows=$data['items']??$data['posts']??$data['results']??$data;if(!is_array($rows))return[];$out=[];foreach($rows as $n){if(!is_array($n))continue;$url=$n['url']??$n['link']??$n['permalink']??null;$title=$n['title']??$n['name']??null;if(is_array($title))$title=$title['rendered']??'';if(!$url||!$title)continue;$out[]=['url'=>$this->absoluteUrl((string)$url,$source->base_url),'title'=>$this->cleanText((string)$title),'summary'=>$this->makeShortSummary(strip_tags((string)($n['excerpt']??$n['description']??''))),'content'=>$this->cleanText(strip_tags((string)($n['content']??$n['body']??$n['description']??''))),'category'=>$this->detectDepartment((string)$title),'published_at'=>$n['date']??$n['published_at']??null];}return array_slice($out,0,30);
    }

    private function parseDetail(string $html,string $url): array
    {
        $dom=$this->dom($html);$xpath=new \DOMXPath($dom);$this->removeNoise($xpath);$title=$this->meta($xpath,'og:title')?:$this->firstText($xpath,'//h1')?:$this->firstText($xpath,'//title');$date=$this->meta($xpath,'article:published_time')?:$this->firstAttr($xpath,'//time[@datetime]','datetime');$node=null;foreach(['//article','//*[contains(concat(" ",normalize-space(@class)," ")," post-body ")]', '//*[contains(concat(" ",normalize-space(@class)," ")," entry-content ")]', '//*[contains(concat(" ",normalize-space(@class)," ")," post-content ")]', '//main'] as $selector){$nodes=$xpath->query($selector);if($nodes&&$nodes->length&&mb_strlen($nodes->item(0)->textContent)>250){$node=$nodes->item(0);break;}}$node=$node?:$dom->documentElement;$content=$this->nodeText($node);$apply=$this->findLink($xpath,'apply|online application|आवेदन');$notification=$this->findLink($xpath,'notification|advertisement|विज्ञापन|pdf');return ['title'=>$this->cleanText($title),'content'=>$content,'summary'=>$this->makeShortSummary($content),'published_at'=>$date,'url'=>$url,'apply_url'=>$apply,'notification_url'=>$notification];
    }

    private function findLink(\DOMXPath $xpath,string $pattern): ?string
    {foreach($xpath->query('//a[@href]') as $a){$label=$this->cleanText($a->textContent);$href=trim($a->getAttribute('href'));if($href&&preg_match('/'.$pattern.'/iu',$label.' '.$href))return $href;}return null;}
