...
class ServerBackedNewsRepository(
    ...
    @Volatile
    private var language: String = ServerConfig.getLanguage(context)
    ...
    fun setLanguage(language: String) {
        ...
        ServerConfig.setLanguage(context, this.language)
        scope.launch { tryFetchFromServer() }
    }
    ...
    suspend fun tryFetchFromServer(): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            try {
                val secResp = api.getSections()
                if (secResp.success && !secResp.sections.isNullOrEmpty()) {
                    _sectionsStream.value = secResp.sections.map { s -> s.toDomain() }
                }
            } catch (e: Exception) {
                Log.w("CGJobsRepo", "sections: ${e.message}")
            }
            try {
                val catResp = api.getCategories(section = null, language = language)
                if (catResp.success && !catResp.categories.isNullOrEmpty()) {
                    _categoriesStream.value = catResp.categories.map { c -> c.toDomain() }
                }
            } catch (e: Exception) {
                Log.w("CGJobsRepo", "categories: ${e.message}")
            }
            ...
            if (response.success && !response.news.isNullOrEmpty()) {
                ...
                _newsStream.value = response.news.map { it.toDomain() }.map {
                    if (it.id in saved) it.copy(isSaved = true) else it
                }
            ...
            Log.w("CGJobsRepo", "sync: ${e.message}")
        }
    ...
    override fun getNewsByCategory(category: String): Flow<List<JobUpdate>> {
        return _newsStream.map { list ->
            ...
            if (clean == "सभी" || clean.equals("All", ignoreCase = true) || clean.startsWith("सभी") || clean.startsWith("All")) {
                ...
                list.filter {
                    it.category.equals(clean, ignoreCase = true) || it.category.contains(clean, ignoreCase = true)
                }
            }
        }
    }
    ...
    override fun getNewsById(id: String): Flow<JobUpdate?> {
        return _newsStream.map { list -> list.find { item -> item.id == id } }
    }
    ...
    override suspend fun toggleSave(id: String) {
        _newsStream.value = _newsStream.value.map {
    ...
    override suspend fun refreshNews() {
        if (!tryFetchFromServer() && _newsStream.value.isEmpty()) {
            fallbackRepository.refreshNews()
            _newsStream.value = fallbackRepository.getNewsStream().first()
        }
    }
}
