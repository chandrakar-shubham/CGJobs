...
class NewsStudioController extends Controller
{
    ...
    public function clearBatch()
    {
        ...
        return back()->with('success','AI Batch cleared. News articles were not deleted.');
    }
    ...
    {
        $news->update(['status'=>'archived','published_web'=>false,'published_mobile'=>false]);
        AiContent::where('source_type','news')->where('source_id',$news->id)->where('status','published')->update(['status'=>'generated','published_at'=>null]);
        return back()->with('success','News article archived.');
    }
    ...
    public function unpublish(News $news)
    {
        $news->update(['status'=>'archived','published_web'=>false,'published_mobile'=>false]);
        AiContent::where('source_type','news')->where('source_id',$news->id)->where('status','published')->update(['status'=>'generated','published_at'=>null]);
        return back()->with('success','News article unpublished from Website + Mobile/App.');
    }
