...
class Job extends Model
{
    ...
    {
        $language=$language?:request()->query('lang','hi');$language=in_array($language,['hi','en'],true)?$language:'hi';$english=$language==='en';$fallback=fn($en,$hi)=>$english?($en?:$hi):($hi?:$en);
        return ['id'=>$this->custom_id?: (string)$this->id,'language'=>$language,'title'=>$fallback($this->title_en,$this->title),'titleHindi'=>$this->title,'titleEnglish'=>$this->title_en?:$this->title,'summary'=>$fallback($this->summary_en,$this->summary),'detailedContent'=>$fallback($this->detailed_content_en,$this->detailed_content?:$this->summary),'category'=>$fallback($this->category_en,$this->category),'categoryHindi'=>$this->category,'categoryEnglish'=>$this->category_en?:$this->category,'section'=>$this->section?:'jobs','postType'=>$this->post_type?:'job','source'=>$this->source?:'cgstate.gov.in','sourceUrl'=>$this->source_url?:'https://cgstate.gov.in','imageUrl'=>$this->image_url,'publishedAt'=>$this->published_at?:$this->created_at?->format('Y-m-d')?:date('Y-m-d'),'relativeTime'=>$fallback($this->relative_time_en,$this->relative_time?:'हाल ही में'),'isBreaking'=>(bool)$this->is_breaking,'isNew'=>(bool)$this->is_new,'vacancies'=>$this->vacancies,'salary'=>$this->salary,'eligibility'=>$fallback($this->eligibility_en,$this->eligibility),'ageLimit'=>$this->age_limit,'selectionProcess'=>$fallback($this->selection_process_en,$this->selection_process),'officialNotificationUrl'=>$this->official_notification_url,'applyUrl'=>$this->apply_url,'importantDates'=>['applicationStart'=>$this->application_start?:($english?'Open':'जारी'),'lastDate'=>$this->last_date?:($english?'Soon':'शीघ्र'),'examDate'=>$this->exam_date,'admitCardDate'=>$this->admit_card_date,'resultDate'=>$this->result_date]];
    }
}
